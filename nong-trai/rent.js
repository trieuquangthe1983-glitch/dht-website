...
/* ---------------------- DANH MỤC & HẰNG SỐ ---------------------- */
const RENT_PLANS = [
  { id: 'family', icon: '🥬', n: 'Vườn rau gia đình', seg: 'Gia đình đô thị bận rộn', type: 'veg', sop: 'leafy', min: 20, max: 60, def: 30, service: 'full', billing: 'month', price: 40000, priceL: '₫/m²/tháng',
  ...
  { id: 'diy', icon: '🧑‍🌾', n: 'Tự canh tác cuối tuần', seg: 'Người yêu làm vườn, gia đình có trẻ nhỏ', type: 'veg', sop: 'leafy', min: 10, max: 40, def: 20, service: 'diy', billing: 'month', price: 15000, priceL: '₫/m²/tháng',
  ...
  { id: 'flock', icon: '🐔', n: 'Nhận nuôi đàn gà đẻ', seg: 'Gia đình muốn trứng sạch hằng tuần', type: 'poultry', sop: 'layer', min: 10, max: 100, def: 20, service: 'full', billing: 'month', price: 35000, priceL: '₫/con/tháng',
  ...
  { id: 'mush', icon: '🍄', n: 'Kệ nấm sạch', seg: 'Gia đình, nhà hàng nhỏ', type: 'mushroom', sop: 'oyster', min: 100, max: 1500, def: 200, service: 'full', billing: 'season', price: 18000, priceL: '₫/bịch/lứa',
    inc: ['Phôi nấm, nhà nuôi, phun sương tự động', 'Nhận nấm tươi các đợt thu (≈ 0,6 kg/bịch/lứa)', 'Miễn phí 1 workshop cấy nấm'] },
  { id: 'herb', icon: '🌿', n: 'Vườn dược liệu GACP', seg: 'Người quan tâm sức khỏe, phòng khám YHCT', type: 'herb', sop: 'cagaileo', min: 50, max: 1000, def: 100, service: 'full', billing: 'season', price: 60000, priceL: '₫/m²/vụ',
    inc: ['Canh tác theo GACP-WHO', 'Sơ chế, sấy, đóng gói có tem truy xuất', 'Hồ sơ nguồn gốc cho từng lô dược liệu'] },
  { id: 'biz', icon: '🏢', n: 'Nông trại doanh nghiệp (CSR/ESG)', seg: 'Doanh nghiệp, trường học', type: 'veg', sop: 'leafy', min: 150, max: 600, def: 240, service: 'full', billing: 'month', price: 30000, priceL: '₫/m²/tháng',
  ...
  { id: 'share', icon: '🤝', n: 'Hợp tác sản xuất – chia sản lượng', seg: 'Nhà đầu tư, HTX, nông hộ', type: '', sop: '', min: 1, max: 100000, def: 0, service: 'full', billing: 'share', price: 0, sharePct: 40, priceL: 'Không phí thuê · chia sản lượng',
...
function payInvoice(i, date) {
  ...
  S.fin.push({ id: uid(), date: i.paidDate, type: 'in', cat: i.kind === 'deposit' ? 'Tiền đặt cọc thuê' : 'Cho thuê nông trại', amount: i.amount, batchId: i.kind === 'deposit' ? '' : (b ? b.id : ''), note: `${i.code} · ${custName(i.customerId)}` });
}
...
function plotForm(id, unitId) {
  ...
  openForm({
    ...
    fields: d => { const u = get('units', d.unitId) || {}; return [
      ...
      { k: 'status', l: 'Trạng thái', type: 'select', opts: [['free', 'Trống – sẵn sàng cho thuê'], ['maint', 'Đang cải tạo']].concat(p && p.status === 'rented' ? [['rented', 'Đang cho thuê']] : []) },
...
function contractForm(preset = {}) {
  const free = S.plots.filter(p => p.status === 'free');
  if (!free.length) { toast(S.plots.length ? 'Tất cả lô đã được thuê — hãy thêm lô mới' : 'Chưa có lô cho thuê — hãy thêm lô trước'); return; }
  const plan0 = planOf(preset.planId), plot0 = get('plots', preset.plotId) || free.find(p => plotFits(p, plan0)) || free[0];
  openForm({
    title: 'Ký hợp đồng thuê nông trại', ok: 'Ký hợp đồng & khởi tạo vườn',
    data: { customerId: preset.customerId || (S.customers[0] || {}).id || '_new', planId: plan0.id, plotId: plot0.id, start: today(), service: plan0.service, deposit: 0, sharePct: plan0.sharePct || 0, qty: '', months: '' },
    onChange: (d, k) => {
      const plan = planOf(d.planId);
      if (k === 'planId') { d.service = plan.service; d.sharePct = plan.sharePct || 0; const f = free.find(p => plotFits(p, plan)); if (f) d.plotId = f.id; d.qty = ''; d.months = ''; d.sopId = ''; }
      if (k === 'plotId') { d.qty = ''; d.sopId = ''; }
    },
    fields: d => {
      const plan = planOf(d.planId), plot = get('plots', d.plotId) || free[0], unit = get('units', plot.unitId) || {};
      const sops = SOPS.filter(s => s.type === unit.type);
      if (!sops.find(s => s.id === d.sopId)) d.sopId = (sops.find(s => s.id === plan.sop) || sops[0] || {}).id;
      const sop = SOPS.find(s => s.id === d.sopId) || SOPS[0];
      if (d.qty === '' || d.qty == null) d.qty = plan.def ? Math.min(plot.size, plan.def) : plot.size;
      if (!d.months) d.months = plan.billing === 'month' ? 6 : Math.max(1, Math.ceil(sop.duration / 30));
      return [
        { k: 'customerId', l: 'Khách thuê', type: 'select', re: true, opts: [...opts(S.customers, c => `${c.name} · ${c.type}`), ['_new', '＋ Khách hàng mới']] },
        ...newCustomerFields(d),
        { k: 'planId', l: 'Gói thuê', type: 'select', re: true, opts: RENT_PLANS.map(p => [p.id, p.icon + ' ' + p.n]) },
        { k: 'plotId', l: 'Lô cho thuê', type: 'select', re: true, opts: free.map(p => [p.id, `${p.code} · ${(get('units', p.unitId) || {}).name} · ${nf(p.size)} ${plotUnitL(p)}${plotFits(p, plan) ? '' : ' (không khớp gói)'}`]) },
        { k: 'sopId', l: 'Cây trồng / vật nuôi (quy trình chuẩn)', type: 'select', opts: sops.map(s => [s.id, s.name]) },
        { k: 'qty', l: `Quy mô thuê (${sop.qtyUnit})`, type: 'number', re: true, req: true, min: 1, hint: `Lô tối đa ${nf(plot.size)} ${plotUnitL(plot)}${plan.billing !== 'share' ? ` · gói khuyến nghị ${nf(plan.min)}–${nf(plan.max)}` : ''}` },
        { k: 'service', l: 'Mức dịch vụ', type: 'select', opts: Object.entries(SERVICE) },
        { k: 'start', l: 'Ngày bắt đầu', type: 'date', req: true },
        { k: 'months', l: 'Thời hạn (tháng)', type: 'number', re: true, req: true, min: 1 },
        plan.billing !== 'share' ? { k: 'deposit', l: 'Đặt cọc (₫)', type: 'number', min: 0 } : { k: 'sharePct', l: 'Tỷ lệ sản lượng chia cho khách (%)', type: 'number', min: 0 },
        { k: 'note', l: 'Điều khoản bổ sung', type: 'textarea' }
      ];
    },
    intro: d => {
      const plan = planOf(d.planId), c = { billing: plan.billing, price: plan.price, qty: +d.qty || 0, months: +d.months || 0, sopId: d.sopId };
      const v = contractValue(c);
      return `<div class="alert info"><span class="ic">${plan.icon}</span><div class="grow"><b>${esc(plan.n)}</b> · ${esc(plan.seg)}<br><small>${esc(plan.inc.join(' · '))}</small><br>
        ${plan.billing === 'share' ? `<b>Không thu phí thuê</b> — khách nhận ${nf(d.sharePct || plan.sharePct)}% sản lượng mỗi lần thu hoạch.` : `Đơn giá ${money(plan.price)} ${esc(plan.priceL.replace('₫', ''))} → <b>${money(v)}</b> cho ${nf(c.months)} tháng${plan.billing === 'season' ? ` (${seasons(c)} vụ/lứa)` : ` (${money(plan.price * c.qty)}/tháng)`}.`}
        <br><small>Hệ thống sẽ tự tạo lứa/vụ theo quy trình chuẩn, lịch chăm sóc, lịch hóa đơn và mở Cổng khách thuê.</small></div></div>`;
    },
    submit: d => {
      const plan = planOf(d.planId), plot = get('plots', d.plotId);
      if (!(d.qty > 0) || d.qty > plot.size) return `Quy mô phải trong khoảng 1 – ${nf(plot.size)} ${plotUnitL(plot)}`;
      if (!d.sopId) return 'Khu của lô này chưa có quy trình phù hợp';
      const customerId = takeCustomer(d);
      const c = { id: uid(), code: code('HD', d.start), customerId, planId: plan.id, plotId: plot.id, sopId: d.sopId, qty: d.qty, service: d.service, billing: plan.billing, price: plan.price, months: d.months, start: d.start, end: addMonths(d.start, d.months), deposit: d.deposit || 0, sharePct: plan.billing === 'share' ? (d.sharePct || 0) : 0, status: 'active', batchIds: [], note: d.note || '' };
      S.contracts.push(c); plot.status = 'rented';
      startCycle(c, d.sopId, d.start); genInvoices(c);
      UI.cust = customerId;
      setTimeout(() => { location.hash = '#/portal/' + customerId; }, 0);
    },
    done: 'Đã ký hợp đồng — vườn của khách đã được khởi tạo'
  });
}
...
function renewForm(cid) {
  const c = get('contracts', cid);
  openForm({
    title: 'Gia hạn hợp đồng ' + c.code, ok: 'Gia hạn', data: { add: c.billing === 'month' ? 6 : c.months },
    intro: `<p class="muted" style="margin-top:0">Hết hạn hiện tại: ${fd(c.end)}. Hệ thống tạo thêm hóa đơn cho thời gian gia hạn.</p>`,
    fields: [{ k: 'add', l: 'Gia hạn thêm (tháng)', type: 'number', req: true, min: 1 }],
    submit: d => { const before = c.billing === 'season' ? seasons(c) : c.months; c.months += d.add; c.end = addMonths(c.start, c.months); if (c.status !== 'active') c.status = 'active'; genInvoices(c, before); },
...
function checkoutForm() {
  ...
  openForm({
    title: 'Đặt hàng', ok: 'Xác nhận đặt hàng', data: { customerId: UI.cust || (S.customers[0] || {}).id || '', delivery: 'pickup', pay: 'Chuyển khoản' },
    ...
    fields: d => [
      { k: 'customerId', l: 'Khách hàng', type: 'select', re: true, opts: [['', 'Khách lẻ'], ...opts(S.customers, c => `${c.name}${isMember(c.id) ? ' ⭐ thành viên' : ''}`), ['_new', '＋ Khách hàng mới']] },
...
VIEWS.rent = {
  ...
  render() {
    ...
    const od = S.invoices.filter(i => invStatus(i) === 'overdue'), act = S.contracts.filter(c => c.status === 'active');
    const expiring = act.filter(c => diffDays(t, c.end) <= 30).sort((a, b) => a.end.localeCompare(b.end));
    const tab = UI.rtab;
    const map = S.units.filter(u => S.plots.some(p => p.unitId === u.id)).map(u => `<div class="card" style="margin-bottom:16px"><div class="card-head"><h3>${FARM_TYPES[u.type].icon} ${esc(u.name)}</h3><button class="btn sm" data-act="newPlot" data-id="${u.id}">＋ Lô</button></div><div class="plots">${S.plots.filter(p => p.unitId === u.id).map(p => {
      const c = S.contracts.find(x => x.plotId === p.id && x.status === 'active'), b = c && curBatch(c), s = b && stageOf(sopOf(b), bDay(b));
      return `<button class="plot ${p.status}" data-act="plotClick" data-id="${p.id}"><b>${esc(p.code)}</b><small>${nf(p.size)} ${plotUnitL(p)}</small>${c ? `<span>${esc(custName(c.customerId))}</span><small>${planOf(c.planId).icon} ${esc(s ? s.n : 'Chờ vụ mới')}</small>` : `<span>${p.status === 'maint' ? '🔧 Đang cải tạo' : '✨ Trống'}</span>`}</button>`;
    }).join('')}</div></div>`).join('');
    return `<div class="grid g4">
      ...
      <div class="card kpi"><span class="l">Công nợ quá hạn</span><span class="v ${od.length ? 't-bad' : 't-ok'}">${short(sum(od, i => i.amount))}</span><span class="s muted">${od.length} hóa đơn</span></div>
      <div class="card kpi"><span class="l">Khách thuê</span><span class="v">${new Set(act.map(c => c.customerId)).size}</span><span class="s muted">${expiring.length} hợp đồng hết hạn ≤ 30 ngày</span></div>
    </div>
    ...
    ${expiring.length ? `<div class="card" style="margin-bottom:16px"><h3 style="margin-bottom:8px">⏳ Sắp hết hạn – cần chăm sóc gia hạn</h3>${expiring.map(c => alertHtml({ lv: diffDays(t, c.end) < 0 ? 'warn' : 'info', ic: planOf(c.planId).icon, html: `<b>${esc(custName(c.customerId))}</b> · ${esc(c.code)} · lô ${esc((get('plots', c.plotId) || {}).code)} · hết hạn ${fd(c.end)}`, link: '#/portal/' + c.customerId })).join('')}</div>` : ''}
    ${tab === 'map' ? (map || empty('Chưa có lô cho thuê', '<button class="btn pri" data-act="newPlot">Tạo lô đầu tiên</button>')) + `<div class="legend"><span><i class="lg free"></i>Trống</span><span><i class="lg rented"></i>Đang thuê</span><span><i class="lg maint"></i>Cải tạo</span></div>`
...
function planCard(p) {
  ...
  return `<div class="card plan"><div class="sop-h"><span class="ico">${p.icon}</span><div><b>${esc(p.n)}</b><br><small class="muted">${esc(p.seg)}</small></div></div>
    <div class="price">${p.price ? money(p.price) : 'Chia sản lượng'}<small> ${esc(p.priceL.replace(/^₫/, ''))}</small></div>
    ...
    <button class="btn pri sm" data-act="newContract" data-plan="${p.id}">Tạo hợp đồng theo gói</button></div>`;
}
...
VIEWS.contracts = {
  ...
  render() {
    const tab = UI.ctab;
    const tabs = [['contracts', 'Hợp đồng'], ['customers', 'Khách hàng'], ['invoices', 'Hóa đơn'], ['requests', 'Yêu cầu dịch vụ'], ['plots', 'Danh mục lô']];
    let body = '';
    if (tab === 'contracts') body = tbl(['Mã HĐ', 'Khách thuê', 'Gói', 'Lô', ['Quy mô', 'r'], 'Thời hạn', ['Giá trị', 'r'], 'Trạng thái', ''], S.contracts.slice().sort((a, b) => b.start.localeCompare(a.start)).map(c => {
      const p = get('plots', c.plotId) || {}, pl = planOf(c.planId), dl = diffDays(today(), c.end);
      return `<tr><td><b>${esc(c.code)}</b></td><td><a href="#/portal/${c.customerId}">${esc(custName(c.customerId))}</a></td><td>${pl.icon} ${esc(pl.n)}<br><small class="muted">${esc(SERVICE[c.service])}</small></td><td>${esc(p.code || '')}</td><td class="r num">${nf(c.qty)}</td><td>${fds(c.start)} → ${fd(c.end)}</td><td class="r num">${c.billing === 'share' ? nf(c.sharePct) + '% SL' : short(contractValue(c))}</td>
      <td>${c.status === 'active' ? (dl < 0 ? badge('Quá hạn HĐ', 'b-warn') : badge('Hiệu lực', 'b-ok')) : badge('Đã kết thúc')}</td><td>${acts(c.status === 'active' ? `<button class="btn sm" data-act="renew" data-id="${c.id}">Gia hạn</button><button class="btn sm danger" data-act="endContract" data-id="${c.id}">Kết thúc</button>` : `<button class="btn sm" data-act="renew" data-id="${c.id}">Tái ký</button>`)}</td></tr>`;
    }), 'Chưa có hợp đồng');
    else if (tab === 'customers') body = tbl(['Khách hàng', 'Nhóm', 'Liên hệ', ['HĐ hiệu lực', 'r'], ['Đã chi', 'r'], ['Điểm', 'r'], ''], S.customers.map(c => {
      ...
      return `<tr><td><a href="#/portal/${c.id}"><b>${esc(c.name)}</b></a> ${isMember(c.id) ? badge('⭐ Thành viên', 'b-ok') : ''}${c.note ? `<br><small class="muted">${esc(c.note)}</small>` : ''}</td><td>${esc(c.type)}</td><td>${esc(c.phone || '')}<br><small class="muted">${esc(c.address || '')}</small></td><td class="r num">${S.contracts.filter(x => x.customerId === c.id && x.status === 'active').length}</td><td class="r num">${short(spent)}</td><td class="r num">${nf(custPoints(c.id))}</td><td>${acts(editBtn('editCustomer', c.id), delBtn('customers', c.id))}</td></tr>`;
    }), 'Chưa có khách hàng');
    else if (tab === 'invoices') body = tbl(['Số HĐơn', 'Khách', 'Nội dung', 'Hạn', ['Số tiền', 'r'], 'Trạng thái', ''], S.invoices.slice().sort((a, b) => (a.paid - b.paid) || a.date.localeCompare(b.date)).map(i => `<tr><td>${esc(i.code)}</td><td>${esc(custName(i.customerId))}</td><td>${esc(i.desc)}</td><td>${fd(i.date)}</td><td class="r num">${money(i.amount)}</td><td>${invBadge(i)}${i.paid ? `<br><small class="muted">${fd(i.paidDate)}</small>` : ''}</td><td>${acts(i.paid ? '' : `<button class="btn sm pri" data-act="payInv" data-id="${i.id}">💵 Thu tiền</button>`)}</td></tr>`), 'Chưa có hóa đơn');
    ...
    else body = tbl(['Mã lô', 'Khu', ['Quy mô', 'r'], 'Trạng thái', 'Khách thuê', 'Đặc điểm', ''], S.plots.map(p => { const c = S.contracts.find(x => x.plotId === p.id && x.status === 'active'); return `<tr><td><b>${esc(p.code)}</b></td><td>${esc((get('units', p.unitId) || {}).name || '')}</td><td class="r num">${nf(p.size)} ${plotUnitL(p)}</td><td>${badge({ free: 'Trống', rented: 'Đang thuê', maint: 'Cải tạo' }[p.status], { free: 'b-ok', rented: 'b-info', maint: 'b-warn' }[p.status])}</td><td>${c ? esc(custName(c.customerId)) : ''}</td><td><small>${esc(p.note || '')}</small></td><td>${acts(editBtn('editPlot', p.id), delBtn('plots', p.id))}</td></tr>`; }), 'Chưa có lô');
...
VIEWS.portal = {
  title: id => { const c = get('customers', id || UI.cust); return c ? 'Cổng khách thuê · ' + c.name : 'Cổng khách thuê'; },
  render(arg) {
    const cid = get('customers', arg) ? arg : (get('customers', UI.cust) ? UI.cust : (S.contracts.find(c => c.status === 'active') || S.customers[0] || {}).customerId || (S.customers[0] || {}).id);
    ...
    UI.cust = cid;
    const cs = S.contracts.filter(x => x.customerId === cid).sort((a, b) => (a.status === 'active' ? -1 : 1) - (b.status === 'active' ? -1 : 1));
    ...
    return `<div class="toolbar"><select data-chg="portalCust" aria-label="Chọn khách">${S.customers.map(x => `<option value="${x.id}" ${x.id === cid ? 'selected' : ''}>${esc(x.name)}</option>`).join('')}</select><span class="grow"></span><button class="btn" data-act="newRequest" data-id="${cid}">🙋 Yêu cầu dịch vụ</button><a class="btn" href="#/market">🛒 Chợ nông trại</a><button class="btn pri" data-act="newContract" data-cust="${cid}">＋ Thuê thêm</button></div>
    <div class="card hero-cust"><div><small class="muted">Xin chào</small><h2>${esc(c.name)}</h2><small class="muted">${esc(c.type)} · khách từ ${fd(c.joined)} · ${esc(c.phone || '')}</small></div>
    ...
    ${cs.map(portalGarden).join('') || `<div class="card sec">${empty('Khách chưa có hợp đồng thuê', `<button class="btn pri" data-act="newContract" data-cust="${cid}">Ký hợp đồng</button>`)}</div>`}
...
function portalGarden(c) {
  ...
  } else if (c.status === 'active') prog = alertHtml({ lv: 'info', html: `Vụ trước đã kết thúc. <button class="btn sm pri" data-act="newCycle" data-id="${c.id}">🌱 Bắt đầu vụ mới</button>` });
  return `<div class="card sec"><div class="card-head"><div class="sop-h"><span class="ico">${pl.icon}</span><div><b>${esc(pl.n)} · lô ${esc(plot.code || '')}</b> ${c.status === 'active' ? badge('Hiệu lực', 'b-ok') : badge('Đã kết thúc')}<br><small class="muted">${esc(c.code)} · ${nf(c.qty)} ${b ? esc(sopOf(b).qtyUnit) : ''} · ${esc(SERVICE[c.service])} · ${fd(c.start)} → ${fd(c.end)}${b ? ` · <a href="#/batch/${b.id}">${esc(sopOf(b).name)}, ngày ${bDay(b)}</a>` : ''}${c.billing === 'share' ? ` · nhận ${nf(c.sharePct)}% sản lượng` : ''}</small></div></div>
    ${c.status === 'active' ? acts(`<button class="btn sm" data-act="newRequest" data-id="${c.customerId}" data-contract="${c.id}">🙋 Yêu cầu</button>`, b ? `<button class="btn sm" data-act="newHarvest" data-id="${b.id}">📦 Ghi thu hoạch</button>` : '', `<button class="btn sm" data-act="renew" data-id="${c.id}">Gia hạn</button>`) : ''}</div>
    ${prog}
    <h4 class="sec">🧺 Sản phẩm từ vườn (${past.length} vụ/lứa)</h4>${tbl(['Lô', 'Ngày thu', 'Sản phẩm', ['Sản lượng', 'r'], ['Đã giao', 'r'], ''], lots.slice().reverse().map(l => { const dv = sum(l.moves.filter(m => m.type === 'deliver'), m => m.qty); return `<tr><td><a href="#/trace/${encodeURIComponent(l.code)}"><code>${esc(l.code)}</code></a></td><td>${fd(l.date)}</td><td>${esc(l.product)}</td><td class="r num">${nf(l.qty, 1)} ${esc(l.unit)}</td><td class="r num">${nf(dv, 1)}</td><td>${l.remain > 0 ? `<button class="btn sm" data-act="deliver" data-id="${l.id}" data-cust="${c.customerId}">🚚 Giao khách</button>` : ''}</td></tr>`; }), 'Chưa có thu hoạch')}</div>`;
}
  ...
  render() {
    ...
    <div class="shop-wrap"><div class="shop">${ls.map(l => {
      ...
      return `<div class="card prod"><div class="pic">${esc(l.icon || LIST_KINDS[l.kind].icon)}</div><div class="grow"><b>${esc(l.title)}</b><br><small class="muted">${l.sellerType === 'customer' ? '🧺 ' + esc(custName(l.sellerId)) : '🏡 ' + esc(S.farm.name)}</small>
        ...
        <div style="display:flex;gap:6px;align-items:center;margin-top:8px;flex-wrap:wrap"><small class="muted">Còn ${nf(stock, 1)} ${esc(l.unit)}</small><span class="grow"></span>${blocked ? badge('Cách ly đến ' + fds(blocked), 'b-warn') : stock > inCart ? `<button class="btn sm pri" data-act="addCart" data-id="${l.id}">＋ Giỏ${inCart ? ' (' + nf(inCart) + ')' : ''}</button>` : badge('Hết hàng')}${editBtn('editListing', l.id)}</div></div></div>`;
...
VIEWS.info = {
  ...
  render() {
    ...
    return `<div class="card hero-farm"><div class="grow"><small>🌱 ${esc(f.owner || '')}</small><h2>${esc(f.name)}</h2><p class="slogan">${esc(I.slogan || '')}</p><p>${esc(I.intro || '')}</p>
      ...
      <div class="acts" style="align-self:flex-start"><button class="btn sm" data-act="editInfo">✎ Sửa</button></div></div>
...
VIEWS.events = {
  ...
  render() {
    ...
    return `<div class="grid g2"><div>
      <div class="card-head"><h2>🎉 Sự kiện sắp tới</h2><button class="btn pri sm" data-act="newEvent">＋ Sự kiện</button></div>
      ${up.map(e => { const n = sum(e.regs, r => r.qty); return `<div class="card event" style="margin-bottom:12px"><div class="date"><b>${e.date.slice(8)}</b><small>Th${+e.date.slice(5, 7)}</small></div><div class="grow"><b>${esc(e.title)}</b> ${badge(e.kind, 'b-info')}<br><small class="muted">${esc(e.time || '')} · ${e.price ? money(e.price) + '/người' : 'Miễn phí'} · ${n}/${e.capacity} chỗ</small><p style="font-size:13px;margin:6px 0">${esc(e.desc || '')}</p><div class="prog"><i style="width:${Math.min(100, n / e.capacity * 100).toFixed(0)}%"></i></div>
        <div style="display:flex;gap:6px;margin-top:8px">${n < e.capacity ? `<button class="btn sm pri" data-act="register" data-id="${e.id}">Đăng ký</button>` : badge('Đã đủ chỗ', 'b-warn')}${editBtn('editEvent', e.id)}${delBtn('events', e.id)}</div>${e.regs.length ? `<details class="faq"><summary>${e.regs.length} lượt đăng ký</summary>${e.regs.map(r => `<div style="font-size:13px">${esc(r.name)} · ${r.qty} người ${r.paid ? '✅' : ''}</div>`).join('')}</details>` : ''}</div></div>`; }).join('') || empty('Chưa có sự kiện sắp tới')}
      ${past.length ? `<p class="muted" style="font-size:13px">${past.length} sự kiện đã diễn ra.</p>` : ''}</div>
      <div><div class="card-head"><h2>📣 Thông báo</h2><button class="btn sm" data-act="newNews">＋ Thông báo</button></div>
      ${news.map(n => `<div class="card" style="margin-bottom:12px"><div class="card-head" style="margin-bottom:4px"><b>${n.pinned ? '📌 ' : ''}${esc(n.title)}</b>${acts(editBtn('editNews', n.id), delBtn('news', n.id))}</div><small class="muted">${fd(n.date)}</small><p style="margin:6px 0 0;white-space:pre-line">${esc(n.body)}</p></div>`).join('') || empty('Chưa có thông báo')}</div></div>`;
  }
};
...
Object.assign(ACT, {
  ...
  plotClick: d => { const p = get('plots', d.id), c = S.contracts.find(x => x.plotId === p.id && x.status === 'active'); if (c) location.hash = '#/portal/' + c.customerId; else if (p.status === 'free') contractForm({ plotId: p.id, planId: (RENT_PLANS.find(pl => plotFits(p, pl)) || {}).id }); else plotForm(p.id); },
  newContract: d => contractForm({ planId: d.plan, customerId: d.cust }), renew: d => renewForm(d.id), newCycle: d => cycleForm(d.id),
  ...
  payInv: d => { const i = get('invoices', d.id); if (!confirm(`Xác nhận thu ${money(i.amount)} – ${i.desc}?`)) return; payInvoice(i); save(); render(); toast('Đã thu tiền và ghi sổ tài chính'); },
...
ACT.del = d => {
  if (d.coll === 'plots' && (get('plots', d.id) || {}).status === 'rented') return toast('Lô đang cho thuê — hãy kết thúc hợp đồng trước');
...
function seedExtra(D) {
  S.info = DEFAULT_INFO(); S.rentInit = true;
  ...
  const C = (id, name, type, phone, address, ago, note) => S.customers.push({ id, name, type, phone, address, joined: D(-ago), note: note || '' });
